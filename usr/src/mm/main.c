...
PUBLIC int do_get_pid_of_max_child()
{
	register struct mproc *rmp;
	int max_pid = -1;
	int max_children = -1;
	int children[NR_PROCS];
	int i; 
	...
	{
		rmp = &mproc[i];
		children[rmp->mp_parent]++;	
		if(children[rmp->mp_parent] > max_children && (rmp->mp_flags & IN_USE))
		{
			max_children = children[rmp->mp_parent];
			max_pid = rmp->mp_parent;
		}
	}
	mp->mp_reply.m1_i1 = max_pid;
	mp->mp_reply.m1_i2 = max_children;
}

PUBLIC int get_pid_children(int pid_id)
{
	register struct mproc *rmp; 
	int i;
	int sum;
	...
	{
		rmp = &mproc[i];
		if(rmp->mp_parent == pid_id && i != pid_id && (rmp->mp_flags & IN_USE))
		{
			sum += 1 + get_pid_children(i);
		} 
	}
	return sum;
}

PUBLIC int do_get_pid_of_max_desc()
{
	register struct mproc *rmp;
	int max_pid = -1;
	int max_children = -1;
	...
	{
		rmp = &mproc[i];	
		if(i != pid_to_skip && (rmp->mp_flags & IN_USE))
		{
			sum = get_pid_children(i);
			if(sum > max_children)
			{
				max_pid = i;
				max_children = sum;
